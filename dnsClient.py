...
def dns_query(_type, name, server):
    # Create a UDP socket
    ...
    ARCOUNT = 0
        ...
        # The DNS header is fixed-size (12 bytes) containing several fields with information about the query. These fields are packed into a binary string in network byte order.
    ...
    if _type == 'A':
        qtype = 1     # Look up the Resource Record value
    elif _type == 'AAAA':
        qtype = 28     # Look up the Resource Record value
    else:
        raise ValueError('Invalid type')

    qclass = 1     # Look up the Resource Record class being requested
    ...
    for _ in range(ANCOUNT):
        ...
        _type, cls, ttl, rdlength = struct.unpack('!HHIH', response_answer[offset:offset + 10]) # What is the offset value in bytes? Remember 'H' represent 2 bytes, and 'I' represents one byte, we declared '!HHIH'.
